...
그대로 채운다**. 제목줄은 남아 있어서 확대 배율·닫기·`⤡` 로 창 모드로 돌아갈 수 있다.

- 켜고 끄기 — 제목줄의 `⤢`/`⤡`, 제목줄 더블클릭, 세션 헤더의 `⤢ 전체화면`, 우클릭 메뉴,
  명령 팔레트, `Ctrl+Shift+F`. 터미널을 가득 채운 상태에서도 `Ctrl+Shift+F` 는 앱이 가져가므로
  되돌릴 길이 항상 남는다(`Esc` 는 쓰지 않는다 — 셸과 `vim` 의 키다).
