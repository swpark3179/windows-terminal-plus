...
보지 못하므로 대신 굴려 준다). `Ctrl+휠` 은 그대로 확대로 넘긴다. `vim`·`less`·`tmux` 처럼 **대체
화면** 을 쓰는 동안은 스크롤백이 없으므로 막대도 사라진다. `claude`·`codex` 가 도는 창
(`pane.ai`)에서는 막대를 늘 또렷하게 둔다 — 거기서는 막대가 사실상 유일한 스크롤 수단이다.
